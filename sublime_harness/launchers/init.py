...
urlopen('http://localhost:7070', data=('init is running').encode('utf8'))
...
class SublimeHarnessInitLauncherNamespaceCommand(sublime_plugin.ApplicationCommand):
    def run(self):
        ...
        urlopen('http://localhost:7070', data=('executing code').encode('utf8'))
        urlopen('http://localhost:7070', data=('file exists %s' % os.path.exists(run_placeholder_path)).encode('utf8'))
